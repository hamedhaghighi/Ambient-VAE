...
class vaegan(object):
    ...
    save_every, ckp_dir):
        ...
        self.images = tf.placeholder(tf.float32, shape=[None, self.output_size, self.output_size, self.channel], name="Inputs")
        ...
        self.next_x, self.training_init_op = self.make_dataset(data_ob.train_data_list)
        self.next_x_val, self.val_init_op = self.make_dataset(data_ob.val_data_list)
    ...
    def build_model_vaegan(self):
        ...
        self.x_lossy = arch.get_lossy(self.hparams, self.mdevice, self.images, self.theta_ph)
        ...
        self.x_tilde_lossy = arch.get_lossy(self.hparams, self.mdevice, self.x_tilde, self.theta_ph_rec)

        self.l_x_tilde , self.De_pro_tilde = self.discriminate(self.x_tilde_lossy)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            summary_writer_train = tf.summary.FileWriter('{}/{}/train'.format(self.log_dir, now), sess.graph)
            summary_writer_test = tf.summary.FileWriter('{}/{}/test'.format(self.log_dir, now), sess.graph)
            step = 0
            if self.load_type!= 'none' and os.path.exists(self.ckp_dir + '/' + self.load_type):
                ckpt = tf.train.get_checkpoint_state(self.ckp_dir, latest_filename=self.load_type)
                if ckpt and ckpt.model_checkpoint_path:
                    self.saver.restore(sess, ckpt.model_checkpoint_path)
                    g_step = int(ckpt.model_checkpoint_path.split('/')[-1].split('-')[-1])
            ...
            while step <= self.max_iters:
                ...
                theta_val_rec = self.mdevice.sample_theta(self.hparams,self.batch_size)
                theta_val_xp = self.mdevice.sample_theta(self.hparams,self.batch_size)
                # next_x_images = sess.run(self.next_x)
                # next_x_images = np.reshape(next_x_images,[-1,28,28,1])
                fd ={self.images: next_x_images, self.theta_ph: theta_val, self.theta_ph_rec: theta_val_rec, self.theta_ph_xp: theta_val_xp}
                ...
                if (step+1) % self.print_every == 0:
                    ...
                    tags = ['D_loss', 'G_loss', 'E_loss', 'PL_loss','L2_loss', 'kl_loss','recon_loss' , 'Learning_rate']
                    ...
                    rec_images, lossy_images , generated_image , rc = sess.run([self.x_tilde,self.x_lossy,self.x_p,self.recon_loss], feed_dict=fd_test)
                    loss_list.append(rc)
                    ...
                    if self.hparams.measurement_type == "drop_independent":
                        lossy_images = lossy_images * (1-self.hparams.drop_prob)
                    # summary_writer_test.add_summary(summary_str, step)
                    ...
                    titles = ['orig', 'lossy', 'reconstructed', 'generated' , 'recon_loss']
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            save_images(sample_images[0:self.batch_size], [self.batch_size/8, 8], '{}/train_{:02d}_{:04d}_con.png'.format(self.log_dir, 0, 0))
