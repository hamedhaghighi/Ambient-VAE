...
def get_mdevice(hparams):
    ...
    elif hparams.measurement_type == 'greyscale':
        mdevice = greyscale(hparams)
    else:
...
class DropMaskType2(DropDevice):
    ...
    def patch_mask(self, hparams , batch_size):
        k = hparams.patch_size
        h, w = hparams.image_dims[0:2]
        dims = [batch_size] + self.batch_dims[1:]
        patch_mask = np.ones(dims)
...
class DropPatch(DropMaskType2):
    ...
    def sample_theta(self, hparams,batch_size):
        return self.patch_mask(hparams, batch_size)


class KeepPatch(DropMaskType2):

    def sample_theta(self, hparams , batch_size):
        return 1 - self.patch_mask(hparams, batch_size)
...
class ExtractPatch(MeasurementDevice):
    ...
    def sample_theta(self, hparams ,batch_size):
        k = hparams.patch_size
        h, w = hparams.image_dims[0:2]
        theta_val = np.zeros([batch_size, 2])
        for i in range(batch_size):
            x, y = np.random.choice(h-k), np.random.choice(w-k)
            theta_val[i, :] = [x, y]
        return theta_val
...
class BlurAddNoise(MeasurementDevice):
    ...
    def sample_theta(self, hparams,batch_size):
        theta_val = hparams.additive_noise_std * np.random.randn(*([batch_size] + self.image_dims))
        return theta_val
...
class greyscale(MeasurementDevice):
    ...
    def measure(self, hparams, x, theta_ph):
        x_grey = (x[:,:,:,0:1] + x[:,:,:,1:2] + x[:,:,:,2:3])/3.0
        x_measured = tf.tile(x_grey , [1, 1 , 1 , 3])
        return x_measured
